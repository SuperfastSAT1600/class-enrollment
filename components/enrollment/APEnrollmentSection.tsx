...
function formatPricePerHour(price: number, hours: number): string {
  const perHour = Math.round(price / hours);
  return `${perHour.toLocaleString()}원`;
}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              },
              {
                icon: ShieldCheck,
                title: '맞춤 커리큘럼',
                desc: '학생의 현재 수준과 목표 점수에 맞춰 설계합니다',
              },
              {
                icon: TrendingUp,
                title: '성과형 관리',
